...
func SetupGenesisBlockWithOverride(db ethdb.Database, triedb *triedb.Database, genesis *Genesis, overrides *ChainOverrides) (*params.ChainConfig, common.Hash, error) {
	...
	if genesis != nil {
		...
		// Khi genesis == nil, đọc config từ database
		storedcfg := rawdb.ReadChainConfig(db, stored)
		if storedcfg != nil {
			newcfg = storedcfg
		} else {
			// Fallback based on genesis hash
			switch stored {
			case params.MainnetGenesisHash:
				newcfg = params.MainnetChainConfig
			case params.HoleskyGenesisHash:
				newcfg = params.HoleskyChainConfig
			case params.SepoliaGenesisHash:
				newcfg = params.SepoliaChainConfig
			case params.GoerliGenesisHash:
				newcfg = params.GoerliChainConfig
			default:
				newcfg = params.MainnetChainConfig
			}
		}
...
// Trong configOrDefault method, thêm case cho PoI
func (g *Genesis) configOrDefault(ghash common.Hash) *params.ChainConfig {
    ...
    default:
        // DEFAULT TO PoI FOR CUSTOM NETWORKS
        return &params.ChainConfig{
            ChainID:             big.NewInt(7090),
            HomesteadBlock:      big.NewInt(0),
            PoI: &params.PoIConfig{
                Period: 2,
				Epoch:  30000,
                Alpha:  0.6,
                Beta:   0.4,
...
func (g *Genesis) Commit(db ethdb.Database, triedb *triedb.Database) (*types.Block, error) {
	...
	if config.PoI != nil && len(block.Extra()) < 32+crypto.SignatureLength {
        return nil, errors.New("can't start PoI chain without signers")
    }
